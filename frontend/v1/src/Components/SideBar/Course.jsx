...
export default function CourseDetails() {
  ...
  const item = { hidden: { y: 40, opacity: 0 }, show: { y: 0, opacity: 1, transition: { type: "spring", stiffness: 120 } } };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <section className="flex-1 p-6 md:p-10 flex items-center justify-center" style={{ maxHeight: sbHeight || "auto" }}>
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 100 }} className="max-w-4xl text-center md:text-left">
            <motion.h1 className="text-4xl md:text-6xl font-extrabold text-gray-800 mb-6" style={{ fontFamily: "'Inter', sans-serif" }}>{deptName}</motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="text-lg md:text-xl text-gray-700 leading-relaxed">{courseData.introduction?.join(" ")}</motion.p>
          </motion.div>
        </section>
      </div>
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <main className="flex-1 px-4 md:px-8 pb-12">
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/*  Major Activities  */}
            <motion.div variants={item} className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
              <h3 className="text-xl font-bold text-gray-800 mb-4">Major Activities</h3>
              <AnimatedList items={courseData.majorActivities} />
            </motion.div>

            {/*  Major Facilities  */}
            <motion.div variants={item} className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
              <h3 className="text-xl font-bold text-gray-800 mb-4">Major Facilities</h3>
              <AnimatedList items={courseData.majorFacilities} />
            </motion.div>

            {/*  Student Association  */}
            <motion.div variants={item} className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
              <h3 className="text-xl font-bold text-gray-800 mb-4">Student Association</h3>
              <AnimatedList items={courseData.associationInfo} className="space-y-2" />
            </motion.div>
          </motion.div>

          {/*  Course Info Table  */}
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ type: "spring", stiffness: 120 }} className="mt-10 overflow-x-auto">
            <table className="w-full text-left border-collapse border border-gray-300 bg-white rounded-2xl shadow-md overflow-hidden">
              <thead className="bg-blue-900 text-white"><tr><th className="p-4 border border-gray-300">Branch</th><th className="p-4 border border-gray-300">Course</th><th className="p-4 border border-gray-300">Intake</th><th className="p-4 border border-gray-300">Since</th></tr></thead>
              <tbody><tr className="bg-gray-50 hover:bg-gray-100 transition"><td className="p-4 border border-gray-300">{courseData.courseInfo.branch}</td><td className="p-4 border border-gray-300">{courseData.courseInfo.courseName}</td><td className="p-4 border border-gray-300"><AnimatedStat value={courseData.courseInfo.intake} /></td><td className="p-4 border border-gray-300">{courseData.courseInfo.establishmentYear}</td></tr></tbody>
            </table>
          </motion.div>
        </main>
      </div>
